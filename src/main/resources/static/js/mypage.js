jQuery(function($){
    ...
    $('.chk_btn').click(function(e){
        ...
        if($(this).attr('class')!='chk_btn act')
        {
            ...
            $.post("ajax.php",{type:'chk2', key:key}, function(data){
               if (data==true)
               {
                   // $(this).addClass('act');
    ...
    $('.rv_smit').click(function(){
        var r_title = $('input[name=r_title]').val(),
            r_link = $('input[name=r_link]').val(),
            r_content = $('textarea[name=r_content]').val();
    ...
    $('.apply_chk').click(function(e){
        ...
        $.post("ajax.php",{type:'chk',obj:obj, key:key}, function(data){
           if (data==true)
           {
               alert('해당 제안서가 채택 되었습니다!');
               location.reload();
           }
           else
           {
    ...
    $('.chat_pop').click(function(){
        ...
        $.post('ajax.php', {type:'chat',obj:obj,key:key}, function(_data){
            ...
            {
                var cont =  "<button title='Close (Esc)' type='button' class='mfp-close'>×</button>"+
                // "<p class='p_name'><span>"+_data.mem_id+"</span> 님과의 소통</p>"+
                "<input type='hidden' name='addCmt_mem_idx' value='"+_data.mem_idx+"'/>"+
                "<input type='hidden' name='addCmt_a_idx' value='"+obj+"'/>"+
                "<div class='cmt_chat teacher_box'>"+
                "<div class='img_bx'>"+_data.mem_img+"</div>"+
                "<div class='cont_bx'>"+
                "<p class='p1'>"+_data.mem_id+"</p>"+
                "<p class='p2'>궁금한게 있다면 여기서 물어보세요.</p>"+
                "</div>"+
                "</div>"+
                _data.lst+
    ...
    $(".addCmt_btn").click(function() {
        ...
        $.ajax({
    		url	: '/main/mypage/addCmt_ajax.php',
    		data : {
                mem_idx : mem_idx,
                a_idx : a_idx,
                content : content,
