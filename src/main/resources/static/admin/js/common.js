jQuery(function(){
    ...
    $(".state_toggle").click(function(e){
        e.preventDefault();
	    var $key = $(this).parents("tr").attr("target");//idx
        var $state = $(this).attr("data-act");//btn 활성타입
        var $trans;
        if($state=="on")
        {
            $trans='B';
        ...
        $.post("ajax.php",{type:"state",key:$key,trans:$trans}, function(data){
           if (data==true)
           {
               location.reload();
           }
    ...
    // state toggle
    $(".state_toggle2").click(function(e){
        e.preventDefault();
	    var $key = $(this).parents("tr").attr("target");//idx
        var $state = $(this).attr("data-act");//btn 활성타입
        var $trans;
        ...
        $.post("ajax.php",{type:"state2",key:$key,trans:$trans}, function(data){
           if (data==true)
           {
               location.reload();
           }
